define([
	...
	var cytraconBuilderService = function($rootScope, cytraconBuilderUrl, elementManager) {
		var self = this;
		self.data = {};
		self.cacheRequests = {};
		...
		this.getBuilderConfig = function(requestKey, callbackFunction) {
			...
			if (!self.data.hasOwnProperty(requestKey)) {
				if (!self.cacheRequests.hasOwnProperty(requestKey)) {
					$.ajax({
		                url: cytraconBuilderUrl.getUrl('mgzbuilder/ajax/loadConfig'),
		                type:'POST',
		                data: {
        ...
        this.elemPost = function(elem, url, data, isJson, successFunction, errorFunction, finallyFunction) {
        	...
        	$.ajax({
                url: cytraconBuilderUrl.getUrl(url),
                type: 'POST',
                data: data,

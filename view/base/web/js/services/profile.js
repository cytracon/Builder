define([
	...
	var profileService = function($rootScope, elementManager, cytraconBuilderForm, cytraconBuilderService) {

		var self = this;
		var defaultSettings = [];

		cytraconBuilderForm.getForm('modals.settings', function(tabs, res) {
		...
		this.export = function() {
			...
			if (cytraconBuilderService.isJSON(_content)) {
				var key = this.getKey();
				if (key) {
		...
		this.import = function() {
			...
			var isJSON  = cytraconBuilderService.isJSON(content);
		...
		this.prepareProfile = function(content, force) {
			...
			if (!profile['pid']) profile['pid'] = cytraconBuilderService.getUniqueId();
			return profile;
		}
		...
		this.toString = function() {
			...
			if (!result['pid']) result['pid'] = cytraconBuilderService.getUniqueId();

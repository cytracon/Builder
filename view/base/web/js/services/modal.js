define([
	...
	var modal = function($rootScope, $uibModal, cytraconBuilderUrl) {
		...
		this.open = function(name, config, closeCallback, cancelCallback) {
			...
			if (modal) {
				...
				config.windowTemplateUrl = cytraconBuilderUrl.getViewFileUrl(config.windowTemplateUrl ? config.windowTemplateUrl : 'Cytracon_Builder/js/templates/modal/window.html');
				...
				if (config.templateUrl) config.templateUrl = cytraconBuilderUrl.getViewFileUrl(config.templateUrl);

define([
    ...
    var filter = function($rootScope, cytraconBuilderUrl, cytraconBuilderService) {
        var self     = this;
        ...
        self.parseAttributesString = function (attributes) {
            ...
            attributes.gsub(
                /(\w+)(?:\s*=\s*(?:(?:"((?:\\.|[^"])*)")|(?:'((?:\\.|[^'])*)')|([^>\s]+)))?/,
                function (match) {
                    if (cytraconBuilderService.isBoolean(match[2])) {
                        result[match[1]] = cytraconBuilderService.strToBoolean(match[2]);
                    } else if (cytraconBuilderService.isBoolean(match[4])) {
                        result[match[1]] = cytraconBuilderService.strToBoolean(match[4]);
                    } else {
            ...
            if (mediaDirectiveMatches) {
                mediaDirectiveMatches.forEach(function (mediaDirective) {
                    ...
                    if (urlMatches && typeof urlMatches[1] !== "undefined") {
                        html = html.replace(mediaDirective, cytraconBuilderUrl.getImageUrl(urlMatches[1].replace(/"/g, "").replace(/&quot;/g, "").replace(/'/g, "")));
                    }
                });
            }
            return html;

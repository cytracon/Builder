define([
	...
	var directive = function($rootScope, cytraconBuilderUrl) {
		return {
			replace: true,
			templateUrl: function(elem) {
				return cytraconBuilderUrl.getTemplateUrl(elem, 'Cytracon_Builder/js/templates/builder/element/gmaps.html');
			},
				...
				{ 
					...
					if ($items) {
						angular.forEach($items, function($_item) {
							var newItem = $_item;
							if (newItem['image']) {
								newItem['image'] = cytraconBuilderUrl.getImageUrl($_item['image']);
							}

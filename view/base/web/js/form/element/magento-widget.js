define([
	...
	return {
		controller: function($rootScope, $scope, cytraconBuilderService, cytraconBuilderFilter, $sce) {
			$scope.ajax = true;
			$rootScope.$broadcast('disableSaveBtn');
			cytraconBuilderService.post('mgzbuilder/widget/index', {}, false, function(res) {
			...
			$scope.$watch('html', function(value) {
				if ($scope.model[$scope.options.key] && value) {
	                ...
	                if (widgetValue) {
			            ...
			            cytraconBuilderService.post('mgzbuilder/widget/loadOptions', {
			...
			$scope.updateValue = function() {
				...
				if (form.valid() && $scope.ajax) {
					$scope.ajax = false;
					$rootScope.$broadcast('disableSaveBtn');
					cytraconBuilderService.post('mgzbuilder/widget/buildWidget', form.serialize(), false, function(res) {
						$scope.$apply(function() {
							$scope.ajax = true;
							$rootScope.$broadcast('enableSaveBtn');
							$scope.model[$scope.options.key] = cytraconBuilderFilter.decodeContent(res);
						});
					});
				}
			}

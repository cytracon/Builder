define([
	...
	return {
		...
		controller: function($rootScope, $scope, cytraconBuilderService, elementManager, cytraconBuilderModal, profileManager) {
		    ...
		    $scope.importItem = function(item) {
		    	if (item.file || item.elements.length) {
			    	...
			    	if (item.elements && item.elements.length) {
						...
						cytraconBuilderModal.closeModal();
			    	...
			    	if (item.file) {
			    		item.importing = true;
			    		cytraconBuilderService.post('mgzbuilder/ajax/template', {
							item: item
						}, true, function(res) {

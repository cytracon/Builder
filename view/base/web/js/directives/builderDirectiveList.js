define([
	...
	var directive = function(cytraconBuilderService, $compile) {
		return {
			scope: {
				group: '@',
				tag: '@',
			...
			link: function(scope, element, attrs) {
				...
				var deferreds = [];
				var items     = {};
				angular.forEach(cytraconBuilderService.directives, function(directive, index) {
					...
					if ($.inArray(scope.tag, tags) !== -1 || $.inArray(scope.group, groups) !== -1) {
						if (!directive.hasOwnProperty('disabled') || !directive.disabled) {
							...
							additionalClasses += ' cytracon-builder-directive-' + directive.type;
							if (directive.group) additionalClasses += ' cytracon-builder-directive-group-' + directive.group;
							if (directive['element']) {
								var deferred;
								deferred = $.Deferred();
								deferreds.push(deferred);
								require([directive['element']], function(Directive) {
									var html = '<' + htmlTag + ' class="cytracon-builder-directive' + additionalClasses + '" ' + (directive.templateUrl ? 'templateUrl="' + directive.templateUrl + '"' : '') + ' element="element" dynamic-directive element-name="mgz-directive-' + directive.type + '" sort-order="' + (directive['sortOrder'] ? directive['sortOrder'] : 0) + '"></' + htmlTag + '>';
									items[index] = html;
									deferred.resolve();
								});
							} else {

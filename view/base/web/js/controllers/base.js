define([
	...
	'underscore',
	'angular',
	'Cytracon_Builder/js/apply/main',
	'Cytracon_Builder/js/parallax'
	...
	var baseCtrl = function(
		...
		$scope.controls            = [];
		$scope.controlsTemplateUrl = cytraconBuilderUrl.getViewFileUrl('Cytracon_Builder/js/templates/builder/controls.html');
		...
		self.getWrapperClasses = function() {
			...
			if (element.el_float) {
				var _float = cytraconBuilderService.getDesignValue(element, 'el_float');
			...
			var viewMode = cytraconBuilderService.getViewMode();
		...
		self.getViewModeClass = function(elem) {
			...
			if (elem.builder && elem.builder.resizable) {
				var responsiveClass = cytraconBuilderService.getResponiveValue(elem, 'size');
				if (responsiveClass) {
					classes.push('mgz-col-xs-' + responsiveClass);
				} else {
					classes.push('mgz-col-xs-12');
				}
				var modeOffset = cytraconBuilderService.getResponiveValue(elem,'offset_size');
		...
		$scope.getTrustedHtml = function(html) {
			return $sce.trustAsHtml(cytraconBuilderFilter.encodeContent(html));
		}
		...
		self.onMouseEnter = function(e) {
			if (!$rootScope.draggingElement || ($('.mgz-element-editing').length && $('.mgz-element-editing') !== $scope.getEl())) {
				if (!$scope.toolbarLoaded && $scope.toolbar) {
					var html = $compile('<cytracon-builder-element-actions></cytracon-builder-element-actions>')($scope);
					$scope.getEl().prepend(html);
					$scope.toolbarLoaded = true;
				}
				if (!$scope.resizableLoaded && $scope.resizable && element.builder.resizable) {
					var html = $compile('<cytracon-builder-element-resizable></cytracon-builder-element-resizable>')($scope);
		...
		self.editElement = function(elem, activeTab) {
			...
			var modal = cytraconBuilderModal.open('element', {
		...
		self.loadLiveElement = function(elem) {
			if (elem.builder.livePreview) {
				cytraconBuilderService.elemPost(elem, $rootScope.builderConfig.loadElementUrl, {
					...
					if (res) {
						...
						$('body').trigger('cytraconPageBuilderUpdated');
						setTimeout(function() {
		...
		self.getParallaxClasses = function() {
			var parallax      = cytraconBuilderService.getDesignValue(element, 'parallax_type');
			var mouseParallax = cytraconBuilderService.getDesignValue(element, 'mouse_parallax');
			var classes  = [];
		...
		self.initParallax = function() {
			...
			var _elem           = element;
			var backgroundType  = cytraconBuilderService.getDesignValue(_elem, 'background_type');
			var imageBgSize     = '';
			var imageBgPosition = cytraconBuilderService.getDesignValue(_elem, 'background_position').replace(/-/g, ' ');
			var video           = false;
			...
			var parallax        = cytraconBuilderService.getDesignValue(_elem, 'parallax_type');
			var parallaxSpeed   = cytraconBuilderService.getDesignValue(_elem, 'parallax_speed');
			var parallaxMobile  = cytraconBuilderService.getDesignValue(_elem, 'parallax_mobile');

	        // video type
	        if ( backgroundType === 'yt_vm_video' || backgroundType === 'video' ) {
	        	video           = cytraconBuilderService.getDesignValue(_elem, 'background_video');
	        	videoStartTime  = parseFloat( cytraconBuilderService.getDesignValue(_elem, 'video_start_time') ) || 0;
	        	videoEndTime    = parseFloat( cytraconBuilderService.getDesignValue(_elem, 'video_end_time') ) || 0;
	        	videoVolume     = parseFloat( cytraconBuilderService.getDesignValue(_elem, 'video_volume') ) || 0;
	        	videoLoop       = true;
	        	videoAlwaysPlay = true;
	        	videoMobile     = cytraconBuilderService.getDesignValue(_elem, 'video_mobile');
	        ...
	        var backgroundImage = cytraconBuilderService.getDesignValue(element, 'background_image');
	        if (backgroundImage) jarallaxParams.imgSrc = cytraconBuilderUrl.getImageUrl(backgroundImage);
	        var elParallax = $scope.getEl().children('.mgz-element-inner').children('.mgz-parallax').children('.mgz-parallax-inner');
		...
		self.isEnabledParallax = function() {
			...
			var parallax        = cytraconBuilderService.getDesignValue(_elem, 'parallax_type');
			var backgroundType  = cytraconBuilderService.getDesignValue(_elem, 'background_type');
			var backgroundImage = cytraconBuilderService.getDesignValue(_elem, 'background_image');
			var backgroundColor = cytraconBuilderService.getDesignValue(_elem, 'background_color');
			var video           = cytraconBuilderService.getDesignValue(_elem, 'background_video');
		...
		self.getParallaxHtml = function() {
			return '<div ng-if="mgz.isEnabledParallax()" ng-class="mgz.getParallaxClasses()" data-parallax-mouse-parallax-size="{{ $root.cytraconBuilderService.getDesignValue(element, \'mouse_parallax_size\') }}" data-parallax-mouse-parallax-speed="{{ $root.cytraconBuilderService.getDesignValue(element, \'mouse_parallax_speed\') }}"><div class="mgz-parallax-inner" after-render="mgz.afterParallaxRender()"></div></div>';
		}
		...
		$scope.getDesignValue = function(key) {
			return cytraconBuilderService.getDesignValue(element, key);	
		}

define([
	...
	var baseCtrl = function(
		...
		var self = this;
		self.id  = '.' + cytraconBuilderModal.getModalId(modal.key);
		...
		if (cytraconBuilderService.getFromLocalStorage(localStorageKey + '.sizes')) $scope.modal.sizes = cytraconBuilderService.getFromLocalStorage(localStorageKey + '.sizes');
		if (cytraconBuilderService.getFromLocalStorage(localStorageKey + '.position')) {
			var position = cytraconBuilderService.getFromLocalStorage(localStorageKey + '.position');
				...
				setTimeout(function() {
			    	self.getModalSelector().resizable({
			    		...
			    		resize: function( event, ui ) {
			    			...
			    			self.resizeInner();
			    			cytraconBuilderService.saveToLocalStorage(localStorageKey + '.sizes', ui.size);
			    		}
			    	});
				}, 2000);
	    		self.getModalSelector().draggable({
	    			...
	    			drag: function( event, ui ) {
	    				if (modal.position.top >= 0) {
				    		...
				    		cytraconBuilderService.saveToLocalStorage(localStorageKey + '.position', ui.position);
				    	}
	    			}
	    		});

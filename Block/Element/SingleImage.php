...
class SingleImage extends \Cytracon\Builder\Block\Element
{
    /**
     * @var array
     */
    ...
    public function getSrcset()
    {
        ...
        switch ($responsiveImages) {
            case 'auto':
                // $tabletImage         = $this->builderImageHelper->resize($src, 1024, null, 100, 'cytracon/resized');
                // $landscapePhoneImage = $this->builderImageHelper->resize($src, 768, null, 100, 'cytracon/resized');
                // $portraitPhoneImage  = $this->builderImageHelper->resize($src, 576, null, 100, 'cytracon/resized');
                // $srcset      = $image . ' 1200w';
    ...
    public function getLinkClasses()
    {
        ...
        $element = $this->getElement();
        $onclick = $element->getData('onclick');
        if (($onclick == 'magnific' || $onclick == 'video_map')) {
            $classes[] = 'mgz-magnific';
        }
        return $this->builderHelper->parseClasses($classes);
    }
    /**
